...
export function useRequisicionCreate(props: CreateProps) {
    ...
    const empleadosForUI = computed(() => {
        ...
        return empleadosActive.value
            ...
            .map((e) => {
                const fullName = normStr(e.nombre ?? e.name ?? '')
                const puesto = normStr(e.puesto ?? e.puesto_nombre ?? '')
                const area = normStr(e.area?.nombre ?? e.area_nombre ?? '')

                const secondary = [puesto, area].filter(Boolean).join(' • ') // <-- aquí
                return {
                    id: Number(e.id),
                    label: fullName,                          // lo que se ve como principal
                    secondary: secondary || undefined,         // segunda línea
                    group: area || 'Sin área',
                }
            })
    })
    ...
    // ==========================================================
    // 8) CALENDAR (FIX TS + FIX toDate) - v-model
    // ==========================================================
    // ==========================================================
    ...
    const solicitanteName = computed(() => {
        ...
        if (!emp) return ''
        // Nombre completo
        const fullName = normStr(emp.nombre ?? emp.name ?? '')
        const puesto = normStr(emp.puesto ?? emp.puesto_nombre ?? '')
        const area = normStr(emp.area?.nombre ?? emp.area_nombre ?? '')
        const puestoTxt = puesto ? ` (${puesto})` : ''
        const areaTxt = area ? ` en ${area}` : ''
        return normStr(`${fullName}${puestoTxt}${areaTxt}`)
    })
