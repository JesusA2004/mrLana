...
const userRole = computed(() => props.dashboard?.userRole ?? 'CONTADOR')
...
const exportPdf = () => downloadFile(route('dashboards.export.pdf', { role: 'CONTADOR' }))
const exportExcel = () => downloadFile(route('dashboards.export.excel', { role: 'CONTADOR' }))
</script>
        ...
        <div class="grid gap-3 lg:grid-cols-2 flex-1 min-h-0">
          <Card class="rounded-2xl border-slate-200/70 dark:border-white/10 overflow-hidden min-h-0">
            <CardHeader class="py-3">
              ...
              <CardDescription class="text-xs">Últimos 30 días (eventos del flujo).</CardDescription>
            </CardHeader>
          ...
          <Card class="rounded-2xl border-slate-200/70 dark:border-white/10 overflow-hidden min-h-0">
            <CardHeader class="py-3">
              ...
              <CardDescription class="text-xs">Últimos 30 días (total diario).</CardDescription>
            </CardHeader>
            <CardContent class="pt-0 h-full min-h-0">
              <div class="h-full min-h-0 w-full relative z-0 overflow-hidden rounded-2xl border border-slate-200/70 dark:border-white/10 bg-white/60 dark:bg-zinc-950/30 p-3">
