...
<template>
    <div ref="rootRef" class="relative min-w-0">
        <label v-if="label" class="block text-xs font-semibold
        text-slate-600 dark:text-neutral-300"
        :for="buttonId" @click.prevent="openFromLabel">
            {{ label }}
        </label>
        ...
        <div v-if="open" :class="[zCls, basePanel, roundedCls, panelClass]">
            <div class="p-3 border-b border-slate-200/70 dark:border-white/10">
                ...
                focus:border-slate-300 dark:border-white/10
            ...
            <div class="max-h-64 overflow-auto p-2 dark:text-neutral-100">
                <button v-if="allowNullEff" type="button"
                @click="pick(null)" class="w-full text-left px-3
                ...
                <button v-for="o in filtered"
                :key="o[valueKey] ?? o.id" type="button"
                @click="pick(o[valueKey])" class="w-full text-left
                px-3 py-2 rounded-2xl text-sm hover:bg-slate-50
