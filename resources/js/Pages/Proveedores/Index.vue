...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="w-full max-w-full min-w-0 px-3 sm:px-6 lg:px-8 py-4 sm:py-6">
            <!-- HERO -->
            <div class="mb-4 rounded-3xl border border-slate-200/70 dark:border-white/10
                bg-white/90 dark:bg-neutral-900/80 backdrop-blur shadow-sm
                px-4 sm:px-6 py-4 sm:py-5 flex flex-col sm:flex-row
                ...
                <div class="min-w-0">
                    ...
                    <!-- Comentado para que no se muestre el headline
                ...
                w-full sm:w-auto">
                    <IconUserPlus :size="22" stroke-width="1.5"/>Nuevo proveedor
                </button>
            </div>
            ...
            <div class="mb-4 rounded-3xl border border-slate-200/70
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-3">
                    ...
                    <div class="lg:col-span-3 min-w-0">
                        ...
                        <select v-model="state.estatus" :class="selectBase + ' rounded-2xl mt-1'">
                    ...
                    <div class="lg:col-span-3 min-w-0">
                        ...
                        <select v-model="state.perPage" :class="selectBase + ' rounded-2xl mt-1'">
                    ...
                    <div class="lg:col-span-6 min-w-0">
                        ...
                        placeholder="Nombre, RFC, correo, banco..."
                        :class="inputBase + ' rounded-2xl mt-1 hover:border-slate-300 dark:hover:border-white/20'"/>
                    </div>
                    ...
                    <div class="lg:col-span-12 flex flex-wrap items-center gap-3 pt-2">
                        ...
                        <div class="ml-auto flex flex-wrap items-center gap-2">
                            ...
                            <button v-if="canDelete" type="button" :disabled="selectedCount < 1"
            ...
            <!-- TABLA (desktop) -->
            <div class="hidden 2xl:block rounded-3xl border border-slate-200/70
                ...
                <table class="w-full table-auto text-sm">
                    <thead class="bg-slate-50 dark:bg-neutral-950/60">
                        <tr class="text-left text-slate-600 dark:text-neutral-300">
                            ...
                            <th class="px-4 py-3 font-semibold cursor-pointer select-none" @click="setSort('nombre_comercial')">
                                Nombre
                            </th>
                        ...
                        hover:bg-slate-50/70 dark:hover:bg-neutral-950/40 transition">
                            ...
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold border"
                                    :class="statusPill(row.estatus)">
                                    <span class="h-1.5 w-1.5 rounded-full bg-current opacity-40"></span>
                                    <span class="truncate">{{ row.estatus || '—' }}</span>
                                </span>
                            </td>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between
                    ...
                    <div class="text-xs text-slate-600 dark:text-neutral-300">
                        Páginas disponibles: <span class="font-semibold">{{ safePagerLinks.length }}</span>
                    </div>
            ...
            <div class="2xl:hidden grid gap-3">
                    ...
                    <div v-for="row in rows" :key="row.id"
                            ...
                            <div class="flex items-start gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2 min-w-0">
                                        ...
                                        px-3 py-1 text-[11px] font-semibold border shrink-0"
                                        :class="statusPill(row.estatus)">
                                            <span class="h-1.5 w-1.5 rounded-full bg-current opacity-40"></span>
                                            <span class="truncate">{{ row.estatus || '—' }}</span>
                                        </span>
                        ...
                        <div class="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-700 dark:text-neutral-200">
                            <div class="rounded-2xl border border-slate-200/70 dark:border-white/10 bg-slate-50/80 dark:bg-neutral-950/40 p-3">
                                <div class="opacity-70">Correo</div>
                                <div class="font-semibold truncate">{{ row.email || '—' }}</div>
                            </div>

                            <div class="rounded-2xl border border-slate-200/70 dark:border-white/10 bg-slate-50/80 dark:bg-neutral-950/40 p-3">
                                <div class="opacity-70">Beneficiario</div>
                                <div class="font-semibold truncate">{{ row.beneficiario || '—' }}</div>
                            </div>
                        </div>
                ...
                bg-white/90 dark:bg-neutral-900/80 backdrop-blur shadow-sm px-4 py-3">
                    <nav class="flex flex-wrap gap-2 max-w-full justify-center">
                        <button v-for="(link, i) in safePagerLinks"
                        :key="`m-${i}-${link.cleanLabel}`" type="button"
                        @click="link.url ? goTo(link.url) : null"
                        :disabled="!link.url"
                        class="rounded-2xl px-3 py-1.5 text-sm font-semibold border
                        transition border-slate-200 bg-white text-slate-800
                    ...
                    dark:border-white/10 bg-white dark:bg-neutral-900 shadow-2xl">
                        <div class="px-6 py-5 border-b border-slate-200/70 dark:border-white/10">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                dark:bg-white/10 dark:hover:bg-white/15 text-slate-800 dark:text-neutral-100" @click="closeModal">
                                    Cerrar
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="px-6 py-5 grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div class="md:col-span-2">
                                ...
                                <input v-model="form.nombre_comercial" type="text" :class="inputBase + ' rounded-2xl mt-1'" />
                                <div v-if="form.errors.nombre_comercial" class="mt-1 text-xs text-rose-600">
                            ...
                            <div>
                                ...
                                <input v-model="form.rfc" type="text" :class="inputBase + ' rounded-2xl mt-1'" />
                                <div v-if="form.errors.rfc" class="mt-1 text-xs text-rose-600">{{ form.errors.rfc }}</div>
                            </div>

                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-neutral-300" for="corr">Correo</label>
                                <input v-model="form.email" type="email" :class="inputBase + ' rounded-2xl mt-1'" id="corr"/>
                                <div v-if="form.errors.email" class="mt-1 text-xs text-rose-600">{{ form.errors.email }}</div>
                            </div>

                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-neutral-300" for="ben">Beneficiario</label>
                                <input v-model="form.beneficiario" type="text" :class="inputBase + ' rounded-2xl mt-1'" id="ben" />
                                <div v-if="form.errors.beneficiario" class="mt-1 text-xs text-rose-600">{{ form.errors.beneficiario }}</div>
                            </div>

                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-neutral-300">Banco</label>
                                <input v-model="form.banco" type="text" :class="inputBase + ' rounded-2xl mt-1'" />
                            ...
                            <div>
                                ...
                                <input v-model="form.cuenta" type="text" :class="inputBase + ' rounded-2xl mt-1'" />
                                <div v-if="form.errors.cuenta" class="mt-1 text-xs text-rose-600">{{ form.errors.cuenta }}</div>
                            </div>

                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-neutral-300">CLABE</label>
                                <input v-model="form.clabe" type="text" :class="inputBase + ' rounded-2xl mt-1'" />
                                <div v-if="form.errors.clabe" class="mt-1 text-xs text-rose-600">{{ form.errors.clabe }}</div>
                            </div>

                            <div>
                                <label class="block text-xs font-semibold text-slate-600 dark:text-neutral-300">Estatus</label>
                                <select v-model="form.estatus" :class="selectBase + ' rounded-2xl mt-1'">
                                <option value="ACTIVO">ACTIVO</option>
                                <option value="INACTIVO">INACTIVO</option>
                                </select>
                                <div v-if="form.errors.estatus" class="mt-1 text-xs text-rose-600">{{ form.errors.estatus }}</div>
                            </div>
                        </div>
